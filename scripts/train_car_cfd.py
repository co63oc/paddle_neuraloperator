...
pipe = ConfigPipeline(
    [
        YamlConfig("./car_cfd_config.yaml", config_name=config_name, config_folder="../config"),
...
if config.wandb.log and is_logger:
    ...
    if config.wandb.name:
        ...
        wandb_name = "_".join(f"{var}" for var in [config_name, config.data.sdf_query_resolution])
...
if config.opt.scheduler == "ReduceLROnPlateau":
    ...
    tmp_lr = paddle.optimizer.lr.CosineAnnealingDecay(
        T_max=config.opt.scheduler_T_max, learning_rate=optimizer.get_lr()
    )
    optimizer.set_lr_scheduler(tmp_lr)
    scheduler = tmp_lr
...
if config.opt.training_loss == "l2":
    ...
    raise ValueError(f"Got config.opt.training_loss={config.opt.training_loss!r}")
...
class CFDDataProcessor(DataProcessor):
    ...
    def preprocess(self, sample):
        ...
        inward_normals = -sample["triangle_normals"].squeeze(axis=0).to(self.device)
        flow_normals = paddle.zeros(shape=(tuple(sample["triangle_areas"].shape)[1], 3)).to(
            self.device
        )
        flow_normals[:, 0] = -1.0
        batch_dict = dict(
