...
pipe = ConfigPipeline(
    [
        YamlConfig("./uqno_config.yaml", config_name="default", config_folder="../config"),
...
solution_dataset = neuralop.data.datasets.darcy.DarcyDataset(
...
test_loaders = {
    (421): paddle.io.DataLoader(
        dataset=test_db, shuffle=False, batch_size=config.data.test_batch_size
    )
}
...
residual_train_db = neuralop.data.datasets.tensor_dataset.TensorDataset(
    **train_db[
        config.data.n_train_solution : config.data.n_train_solution + config.data.n_train_residual
    ]
)
...
optimizer = paddle.optimizer.Adam(
    parameters=solution_model.parameters(),
    learning_rate=config.opt.solution.learning_rate,
...
if config.opt.solution.scheduler == "ReduceLROnPlateau":
    ...
    tmp_lr = paddle.optimizer.lr.CosineAnnealingDecay(
        T_max=config.opt.solution.scheduler_T_max, learning_rate=optimizer.get_lr()
    )
    optimizer.set_lr_scheduler(tmp_lr)
    scheduler = tmp_lr
...
solution_train_loader = paddle.io.DataLoader(
    dataset=solution_train_db,
    batch_size=config.data.batch_size,
    shuffle=True,
    num_workers=1,
)
...
if config.opt.solution.n_epochs > 0:
    if config.opt.solution.resume is True:
...
def loader_to_residual_db(model, data_processor, loader, device, train_val_split=True):
    ...
    for idx, sample in enumerate(loader):
        ...
        del sample, out
    errors = paddle.concat(error_list, axis=0)
    xs = paddle.concat(x_list, axis=0)  # check this
    ...
    if train_val_split:
        ...
        residual_train_db = neuralop.data.datasets.tensor_dataset.TensorDataset(
            x=xs[:val_start], y=errors[:val_start]
        )
        residual_val_db = neuralop.data.datasets.tensor_dataset.TensorDataset(
            x=xs[val_start:], y=errors[val_start:]
        )
    else:
...
residual_optimizer = paddle.optimizer.Adam(
    parameters=residual_model.parameters(),
    learning_rate=config.opt.residual.learning_rate,
...
residual_train_loader_unprocessed = paddle.io.DataLoader(
    dataset=residual_train_db, batch_size=1, shuffle=True, num_workers=0
)
...
residual_train_loader = paddle.io.DataLoader(
    dataset=processed_residual_train_db,
    batch_size=config.data.batch_size,
    shuffle=True,
    num_workers=0,
)
residual_val_loader = paddle.io.DataLoader(
    dataset=processed_residual_val_db,
    batch_size=config.data.batch_size,
    shuffle=True,
    num_workers=0,
)
...
if config.opt.residual.scheduler == "ReduceLROnPlateau":
    ...
    tmp_lr = paddle.optimizer.lr.CosineAnnealingDecay(
        T_max=config.opt.residual.scheduler_T_max,
        learning_rate=residual_optimizer.get_lr(),
    )
    residual_optimizer.set_lr_scheduler(tmp_lr)
    resid_scheduler = tmp_lr
...
val_ratio_list = []
calib_loader = paddle.io.DataLoader(dataset=residual_calib_db, shuffle=True, batch_size=1)
with paddle.no_grad():
...
def eval_coverage_bandwidth(test_loader, alpha, device="cuda"):
    ...
    with paddle.no_grad():
        for _, sample in enumerate(test_loader):
            sample = {k: v.to(device) for k, v in sample.items() if paddle.is_tensor(x=v)}
            sample = uqno_data_proc.preprocess(sample)
            out = uqno(**sample)
            uncertainty_pred, sample = uqno_data_proc.postprocess(out, sample)
            pointwise_true_err = sample["y"]
            in_pred = (
                (paddle.abs(x=pointwise_true_err) < paddle.abs(x=uncertainty_pred))
                .astype(dtype="float32")
                .squeeze()
            )
            avg_interval = (
                paddle.abs(x=uncertainty_pred.squeeze())
                .view(tuple(uncertainty_pred.shape)[0], -1)
                .mean(axis=1)
            )
            avg_interval_list.append(avg_interval.to("cpu"))
            in_pred_flattened = in_pred.view(tuple(in_pred.shape)[0], -1)
            in_pred_instancewise = paddle.mean(x=in_pred_flattened, axis=1) >= 1 - alpha
            in_pred_list.append(in_pred_instancewise.astype(dtype="float32").to("cpu"))
    in_pred = paddle.concat(in_pred_list, axis=0)
    intervals = paddle.concat(avg_interval_list, axis=0)
...
for alpha in [0.02, 0.05, 0.1]:
    for delta in [0.02, 0.05, 0.1]:
        ...
        val_ratios_pointwise_quantile = paddle.topk(
            k=domain_idx + 1, x=val_ratios.view(tuple(val_ratios.shape)[0], -1), axis=1
        ).values[:, -1]
        uncertainty_scaling_factor = paddle.abs(
            x=paddle.topk(k=function_idx + 1, x=val_ratios_pointwise_quantile, axis=0).values[-1]
        )
        ...
        interval, percentage = eval_coverage_bandwidth(
            test_loader=test_loaders[tuple(train_db[0]["x"].shape)[-1]],
            alpha=alpha,
            device=device,
        )
        if config.wandb.log and is_logger:
