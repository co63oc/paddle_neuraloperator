...
model = UNO(in_channels=1, out_channels=1, hidden_channels=64,
    projection_channels=64, uno_out_channels=[32, 64, 64, 64, 32],
    uno_n_modes=[[16, 16], [8, 8], [8, 8], [8, 8], [16, 16]], uno_scalings=
    [[1.0, 1.0], [0.5, 0.5], [1, 1], [2, 2], [1, 1]], horizontal_skips_map=
    None, n_layers=5, domain_padding=0.2)
...
for index in range(3):
    ...
    x = data['x']
    y = data['y']
    out = model(x.unsqueeze(axis=0).to(device)).cpu()
