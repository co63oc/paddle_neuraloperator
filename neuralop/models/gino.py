...
class GINO(paddle.nn.Layer):
    ...
    def __init__(self, in_channels, out_channels, projection_channels=256,
        ...
        100000, gno_radius=0.033, in_gno_channel_mlp_hidden_layers=[80, 80,
        80], out_gno_channel_mlp_hidden_layers=[512, 256],
        gno_channel_mlp_non_linearity=paddle.nn.functional.gelu,
        in_gno_transform_type='linear', out_gno_transform_type='linear',
        gno_use_open3d=False, gno_use_torch_scatter=True, out_gno_tanh=None,
        fno_in_channels=3, fno_n_modes=(16, 16, 16), fno_hidden_channels=64,
        lifting_channels=256, fno_n_layers=4, fno_output_scaling_factor=
        ...
        self.in_coord_dim_reverse_order = [(j + 2) for j in self.
        ...
        self.lifting = ChannelMLP(in_channels=self.fno_in_channels,
            hidden_channels=lifting_channels, out_channels=
            fno_hidden_channels, n_layers=3)
        ...
        if gno_coord_embed_dim is not None:
            self.pos_embed = SinusoidalEmbedding(in_channels=gno_coord_dim,
                num_frequencies=gno_coord_embed_dim, embedding_type=
                'transformer', max_positions=gno_embed_max_positions)
        ...
        if (in_gno_transform_type == 'nonlinear' or in_gno_transform_type ==
        ...
        self.gno_in = IntegralTransform(channel_mlp_layers=
            in_gno_channel_mlp_hidden_layers, channel_mlp_non_linearity=
            gno_channel_mlp_non_linearity, transform_type=
            in_gno_transform_type, use_torch_scatter=gno_use_torch_scatter)
        out_kernel_in_dim = 2 * self.gno_coord_dim_embed
        out_kernel_in_dim += (fno_hidden_channels if out_gno_transform_type !=
            'linear' else 0)
        out_gno_channel_mlp_hidden_layers.insert(0, out_kernel_in_dim)
        out_gno_channel_mlp_hidden_layers.append(fno_hidden_channels)
        self.gno_out = IntegralTransform(channel_mlp_layers=
            out_gno_channel_mlp_hidden_layers, channel_mlp_non_linearity=
            gno_channel_mlp_non_linearity, transform_type=
            out_gno_transform_type, use_torch_scatter=gno_use_torch_scatter)
        self.projection = ChannelMLP(in_channels=fno_hidden_channels,
            out_channels=self.out_channels, hidden_channels=
            projection_channels, n_layers=2, n_dim=1, non_linearity=
            fno_non_linearity)

    def latent_embedding(self, in_p, ada_in=None):
        in_p = in_p.transpose(perm=[0, len(tuple(in_p.shape)) - 1, *list(
            range(1, len(tuple(in_p.shape)) - 1))])
        if ada_in is not None:
            if ada_in.ndim == 2:
                ada_in = ada_in.squeeze(axis=0)
    ...
    def integrate_latent(self, in_p, out_p, latent_embed):
        ...
        latent_embed = latent_embed.transpose(perm=[0, *self.
            in_coord_dim_reverse_order, 1]).reshape(batch_size, -1, self.
            fno_hidden_channels)
        if self.out_gno_tanh in ['latent_embed', 'both']:
            latent_embed = paddle.nn.functional.tanh(x=latent_embed)
        out = self.gno_out(y=in_p_embed, neighbors=in_to_out_nb, x=
            out_p_embed, f_y=latent_embed)
        out = out.transpose(perm=[0, 2, 1])
        out = self.projection(out).transpose(perm=[0, 2, 1])
        return out

    def forward(self, x, input_geom, latent_queries, output_queries, ada_in
        =None, **kwargs):
        ...
        input_geom = input_geom.squeeze(axis=0)
        latent_queries = latent_queries.squeeze(axis=0)
        spatial_nbrs = self.nb_search_out(input_geom, latent_queries.view((
            -1, tuple(latent_queries.shape)[-1])), radius=self.gno_radius)
        in_p = self.gno_in(y=input_geom, x=latent_queries.view((-1, tuple(
            latent_queries.shape)[-1])), f_y=x, neighbors=spatial_nbrs)
        grid_shape = tuple(latent_queries.shape)[:-1]
        in_p = in_p.view((batch_size, *grid_shape, self.fno_in_channels))
        latent_embed = self.latent_embedding(in_p=in_p, ada_in=ada_in)
        out = self.integrate_latent(latent_queries, output_queries,
            latent_embed)
        return out
