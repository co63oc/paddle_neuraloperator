...
class GINO(paddle.nn.Layer):
    ...
    gno_channel_mlp_non_linearity : nn.Layer, optional
    ...
    gno_use_open3d : bool, optional
        ...
        if False, uses pure-Paddle fallback neighbor search
    gno_use_paddle_scatter : bool, optional
        whether to use paddle_scatter's neighborhood reduction function
        or the native Paddle implementation in IntegralTransform layers.
        If False, uses the fallback Paddle version.
    ...
    fno_non_linearity : nn.Layer, defaults to F.gelu
        nonlinear activation function between each FNO layer.
    fno_stabilizer : nn.Layer | None, defaults to None
        By default None, otherwise tanh is used before FFT in the FNO block.
    fno_norm : nn.Layer | None, defaults to None
    ...
    fno_conv_module : nn.Layer, defaults to SpectralConv
    ...
    def __init__(
        ...
        fno_non_linearity=paddle.nn.functional.gelu,
        fno_stabilizer=None,
        fno_norm=None,
        ...
        **kwargs,
        ...
        if gno_coord_embed_dim is not None:
            self.pos_embed = SinusoidalEmbedding(
                in_channels=gno_coord_dim,
        ...
        self.gno_in = IntegralTransform(
            ...
            use_paddle_scatter=gno_use_paddle_scatter,
        )
        ### output GNO
    ...
    def latent_embedding(self, in_p, ada_in=None):
        ...
        in_p = in_p.transpose(
            perm=[
                0,
                len(tuple(in_p.shape)) - 1,
                *list(range(1, len(tuple(in_p.shape)) - 1)),
            ]
        )
        ...
        if ada_in is not None:
            ...
            if self.adain_pos_embed is not None:
                ada_in_embed = self.adain_pos_embed(ada_in.unsqueeze(axis=0)).squeeze(axis=0)
            else:
                ada_in_embed = ada_in
            if self.fno_norm == "ada_in":
    ...
    def integrate_latent(self, in_p, out_p, latent_embed):
        ...
        in_to_out_nb = self.nb_search_out(
            in_p.view(-1, tuple(in_p.shape)[-1]), out_p, self.gno_radius
        )
        # Embed input points
        ...
        latent_embed = latent_embed.transpose(
            perm=[0, *self.in_coord_dim_reverse_order, 1]
        ...
        out = self.gno_out(y=in_p_embed, neighbors=in_to_out_nb, x=out_p_embed, f_y=latent_embed)
    ...
    def forward(self, x, input_geom, latent_queries, output_queries, ada_in=None, **kwargs):
        ...
        spatial_nbrs = self.nb_search_out(
            input_geom,
            latent_queries.view((-1, tuple(latent_queries.shape)[-1])),
            radius=self.gno_radius,
        )
        in_p = self.gno_in(
            y=input_geom,
            x=latent_queries.view((-1, tuple(latent_queries.shape)[-1])),
            f_y=x,
            neighbors=spatial_nbrs,
        )
        grid_shape = tuple(latent_queries.shape)[:-1]  # disregard positional encoding dim
