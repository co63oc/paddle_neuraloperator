...
class FNOGNO(BaseModel, name="FNOGNO"):
    ...
    gno_channel_mlp_non_linearity : nn.Layer, defaults to F.gelu
        ...
        **kwargs,
        ...
        if gno_coord_embed_dim is not None:
            self.pos_embed = SinusoidalEmbedding(
                in_channels=self.gno_coord_dim,
    ...
    def latent_embedding(self, in_p, f, ada_in=None):
        ...
        if self.gno_batched:
            ...
            in_p = in_p.transpose(perm=[0, -1, *self.in_coord_dim_forward_order])
        else:
            in_p = in_p.transpose(perm=[-1, *self.in_coord_dim_forward_order]).unsqueeze(axis=0)
    ...
    def integrate_latent(self, in_p, out_p, latent_embed):
        ...
        if self.gno_batched:
            ...
            latent_embed = latent_embed.transpose(
                perm=[*self.in_coord_dim_reverse_order, 0]
            ).reshape((-1, self.fno_hidden_channels))
        # (n_out, fno_hidden_channels)
        out = self.gno(y=in_p_embed, neighbors=in_to_out_nb, x=out_p_embed, f_y=latent_embed)
        # if self.gno is variable and not batched
        if out.ndim == 2:
            out = out.unsqueeze(axis=0)
        out = out.transpose(perm=[0, 2, 1])  # b, c, n_out
