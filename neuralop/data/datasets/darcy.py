...
class DarcyDataset(PTDataset):
    ...
    def __init__(
        ...
        if download:
            ...
            for res in resolutions:
                if (
                    f"darcy_train_{res}.pd" not in already_downloaded_files
                    or f"darcy_test_{res}.pd" not in already_downloaded_files
                ):
...
def load_darcy_flow_small(
    ...
    train_loader = paddle.io.DataLoader(
        dataset=dataset.train_db, batch_size=batch_size, num_workers=0
    )
    test_loaders = {}
    for res, test_bsize in zip(test_resolutions, test_batch_sizes):
        test_loaders[res] = paddle.io.DataLoader(
            dataset=dataset.test_dbs[res],
