...
class DarcyDataset(PTDataset):
    ...
    def __init__(self, root_dir: Union[Path, str], n_train: int, n_tests:
        List[int], batch_size: int, test_batch_sizes: List[int],
        train_resolution: int, test_resolutions: int=[16, 32], encode_input:
        bool=False, encode_output: bool=True, encoding='channel-wise',
        channel_dim=1, subsampling_rate=None, download: bool=True):
        """DarcyDataset
        ...
        if download:
            ...
            download_from_zenodo_record(record_id=zenodo_record_id, root=
                root_dir, files_to_download=files_to_download)
        super().__init__(root_dir=root_dir, dataset_name='darcy', n_train=
            n_train, n_tests=n_tests, batch_size=batch_size,
            test_batch_sizes=test_batch_sizes, train_resolution=
            train_resolution, test_resolutions=test_resolutions,
            encode_input=encode_input, encode_output=encode_output,
            encoding=encoding, channel_dim=channel_dim,
...
def load_darcy_flow_small(n_train, n_tests, batch_size, test_batch_sizes,
    data_root=example_data_root, test_resolutions=[16, 32], encode_input=
    False, encode_output=True, encoding='channel-wise', channel_dim=1):
    dataset = DarcyDataset(root_dir=data_root, n_train=n_train, n_tests=
        n_tests, batch_size=batch_size, test_batch_sizes=test_batch_sizes,
        train_resolution=16, test_resolutions=test_resolutions,
        encode_input=encode_input, encode_output=encode_output, channel_dim
        =channel_dim, encoding=encoding, download=False)
    train_loader = paddle.io.DataLoader(dataset=dataset.train_db,
        batch_size=batch_size, num_workers=0)
    test_loaders = {}
    for res, test_bsize in zip(test_resolutions, test_batch_sizes):
        test_loaders[res] = paddle.io.DataLoader(dataset=dataset.test_dbs[
            res], batch_size=test_bsize, shuffle=False, num_workers=0)
    return train_loader, test_loaders, dataset.data_processor


def load_darcy_pt(n_train, n_tests, batch_size, test_batch_sizes, data_root
    ='./neuralop/data/datasets/data', train_resolution=16, test_resolutions
    =[16, 32], encode_input=False, encode_output=True, encoding=
    'channel-wise', channel_dim=1):
    dataset = DarcyDataset(root_dir=data_root, n_train=n_train, n_tests=
        n_tests, batch_size=batch_size, test_batch_sizes=test_batch_sizes,
        train_resolution=train_resolution, test_resolutions=
        test_resolutions, encode_input=encode_input, encode_output=
        encode_output, encoding=encoding, channel_dim=channel_dim, download
        =False)
    train_loader = paddle.io.DataLoader(dataset=dataset.train_db,
        batch_size=batch_size, num_workers=0)
    test_loaders = {}
