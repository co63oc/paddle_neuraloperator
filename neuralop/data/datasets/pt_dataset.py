from functools import partialmethod  # noqa
...
class PTDataset:
    ...
    def __init__(
        ...
        data = paddle.load(
            Path(root_dir).joinpath(f"{dataset_name}_train_{train_resolution}.pd").as_posix()
        )
        ...
        train_input_indices = tuple(
            [slice(0, n_train, None)] + [slice(None, None, rate) for rate in input_subsampling_rate]
        )
        x_train = (
            data["x"][train_input_indices].unsqueeze(axis=channel_dim).astype("float32").clone()
        )
        train_output_indices = tuple(
            [slice(0, n_train, None)]
            + [slice(None, None, rate) for rate in output_subsampling_rate]
        )
        y_train = data["y"][train_output_indices].unsqueeze(axis=channel_dim).clone()
        del data
        ...
        for res, n_test in zip(test_resolutions, n_tests):
            ...
            data = paddle.load(
                path=str(Path(root_dir).joinpath(f"{dataset_name}_test_{res}.pd").as_posix())
            )
            # optionally subsample along data indices
            test_input_indices = tuple(
                [slice(0, n_test, None)]
                + [slice(None, None, rate) for rate in input_subsampling_rate]
            )
            x_test = (
                data["x"][test_input_indices].unsqueeze(axis=channel_dim).astype("float32").clone()
            )
            test_output_indices = tuple(
                [slice(0, n_test, None)]
                + [slice(None, None, rate) for rate in output_subsampling_rate]
            )
            y_test = data["y"][test_output_indices].unsqueeze(axis=channel_dim).clone()
            del data
