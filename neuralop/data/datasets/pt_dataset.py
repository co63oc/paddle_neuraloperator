...
class PTDataset:
    ...
    def __init__(self, root_dir: Union[Path, str], dataset_name: str,
        n_train: int, n_tests: List[int], batch_size: int, test_batch_sizes:
        List[int], train_resolution: int, test_resolutions: List[int],
        encode_input: bool=False, encode_output: bool=True, encoding=
        'channel-wise', input_subsampling_rate=None,
        output_subsampling_rate=None, channel_dim=1):
        """PTDataset
        ...
        data = paddle.load(path=str(Path(root_dir).joinpath(
            f'{dataset_name}_train_{train_resolution}.pt').as_posix()))
        ...
        if not isinstance(output_subsampling_rate, list):
            output_subsampling_rate = [output_subsampling_rate
                ] * output_data_dims
        assert len(output_subsampling_rate) == output_data_dims
        train_input_indices = [slice(0, n_train, None)] + [slice(None, None,
            rate) for rate in input_subsampling_rate]
        x_train = data['x'][train_input_indices].unsqueeze(axis=channel_dim
            ).astype('float32').clone()
        train_output_indices = [slice(0, n_train, None)] + [slice(None,
            None, rate) for rate in output_subsampling_rate]
        y_train = data['y'][train_output_indices].unsqueeze(axis=channel_dim
            ).clone()
        del data
        if encode_input:
        ...
        self._data_processor = DefaultDataProcessor(in_normalizer=
            input_encoder, out_normalizer=output_encoder)
        self._test_dbs = {}
        for res, n_test in zip(test_resolutions, n_tests):
            print(
                f'Loading test db for resolution {res} with {n_test} samples ')
            data = paddle.load(path=str(Path(root_dir).joinpath(
                f'{dataset_name}_test_{res}.pt').as_posix()))
            test_input_indices = [slice(0, n_test, None)] + [slice(None,
                None, rate) for rate in input_subsampling_rate]
            x_test = data['x'][test_input_indices].unsqueeze(axis=channel_dim
                ).astype('float32').clone()
            test_output_indices = [slice(0, n_test, None)] + [slice(None,
                None, rate) for rate in output_subsampling_rate]
            y_test = data['y'][test_output_indices].unsqueeze(axis=channel_dim
                ).clone()
            del data
