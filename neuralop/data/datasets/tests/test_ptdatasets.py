...
def test_DarcyDatasetDownload(resolution):
    dataset = DarcyDataset(root_dir=test_data_dir, n_train=5, n_tests=[5],
        batch_size=1, test_batch_sizes=[1], train_resolution=resolution,
        test_resolutions=[resolution, resolution * 2], download=True)
...
def test_BurgersDataset(resolution):
    dataset = Burgers1dTimeDataset(root_dir=burgers_root, n_train=5,
        n_tests=[5], batch_size=1, test_batch_sizes=[1], train_resolution=
        resolution, test_resolutions=[resolution], download=False)
    assert dataset.train_db
    assert dataset.test_dbs
    assert dataset.data_processor
...
def test_NSDownload():
    dataset = NavierStokesDataset(root_dir=test_data_dir, n_train=5,
        n_tests=[5], batch_size=1, test_batch_sizes=[1], train_resolution=
        128, test_resolutions=[128], download=True)
    downloaded_files = os.listdir(test_data_dir)
    for split in ['train', 'test']:
        for res in [128]:
