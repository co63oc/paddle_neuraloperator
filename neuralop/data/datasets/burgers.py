...
class Burgers1dTimeDataset(PTDataset):
    ...
    Attributes
    ----------
    train_db: paddle.io.Dataset of training examples
...
def load_burgers_1d(
    ...
    s = x_train.shape[-1]
    if grid is not None:
        grid = paddle.linspace(start=grid[0], stop=grid[1], num=s + 1)[0:-1].view(1, -1)
        grid_train = grid.tile(repeat_times=[n_train, 1])
        grid_test = grid.tile(repeat_times=[n_test, 1])
        x_train = paddle.concat(x=(x_train.unsqueeze(axis=1), grid_train.unsqueeze(axis=1)), axis=1)
        x_test = paddle.concat(x=(x_test.unsqueeze(axis=1), grid_test.unsqueeze(axis=1)), axis=1)
...
def load_burgers_1dtime(
    ...
    grid_x = paddle.to_tensor(
        data=np.linspace(domain_starts[0], domain_lengths[0], spatial_length + 1)[:-1],
        dtype="float32",
    )
    grid_t = paddle.to_tensor(
        data=np.linspace(domain_starts[1], domain_lengths[1], temporal_length),
        dtype="float32",
    )
    grid_x = grid_x.reshape(1, 1, spatial_length)
    grid_t = grid_t.reshape(1, temporal_length, 1)
    x_train = x_train.reshape(n_train, 1, spatial_length).tile(repeat_times=[1, temporal_length, 1])
    x_test = x_test.reshape(n_test, 1, spatial_length).tile(repeat_times=[1, temporal_length, 1])
    # TODO: add option to not have positional encoding
    x_train = paddle.stack(
        x=[
            x_train,
            grid_t.tile(repeat_times=[n_train, 1, spatial_length]),
            grid_x.tile(repeat_times=[n_train, temporal_length, 1]),
        ],
        axis=3,
    )
    x_test = paddle.stack(
        x=[
            x_test,
            grid_t.tile(repeat_times=[n_test, 1, spatial_length]),
            grid_x.tile(repeat_times=[n_test, temporal_length, 1]),
        ],
        axis=3,
    )
    ...
    train_loader = paddle.io.DataLoader(dataset=train_db, batch_size=batch_size, shuffle=False)
    test_db = TensorDataset(x_test, y_test)
    test_loader = paddle.io.DataLoader(dataset=test_db, batch_size=batch_size_test, shuffle=False)
    output_encoder = None
