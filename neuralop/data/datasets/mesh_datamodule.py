...
class MeshDataModule:
    def __init__(
        ...
        for i, mesh in enumerate(meshes):
            ...
            item_dict["vertices"] = self.range_normalize(item_dict["vertices"], min_b, max_b, 0, 1)
            ...
            if query_points is not None:
                tt = default_timer()
                try:
                    distance, closest = self.compute_distances(mesh, query_points, are_watertight)
                except:  # noqa
        ...
        for data_dict in data:
            for key in data_dict:
                data_dict[key] = paddle.to_tensor(data=data_dict[key]).to("float32")
        # Load non-mesh data
    ...
    def train_dataloader(self, **kwargs):
        return paddle.io.DataLoader(self.train_data, **kwargs)

    def test_dataloader(self, **kwargs):
        return paddle.io.DataLoader(self.test_data, **kwargs)
