...
class MeshDataModule:
    ...
    def __init__(self, root_dir: Union[str, Path], item_dir_name: Union[str,
        Path], n_train: int=None, n_test: int=None, query_res: List[int]=
        None, attributes: List[str]=None):
        ...
        meshes = []
        for ind in mesh_ind:
            mesh = o3d.io.read_triangle_mesh(str(data_dir / (item_dir_name +
                ind + '/tri_mesh.ply')))
            meshes.append(mesh)
        ...
        if isinstance(query_res, list) or isinstance(query_res, tuple):
            ...
            query_points = np.stack(np.meshgrid(tx, ty, tz, indexing='ij'),
        ...
        for i, mesh in enumerate(meshes):
            ...
            centroids, area = self.compute_triangle_centroids(item_dict[
                'vertices'], np.asarray(mesh.triangles))
            item_dict['centroids'] = centroids
            item_dict['triangle_areas'] = area
            item_dict['vertices'] = self.range_normalize(item_dict[
                'vertices'], min_b, max_b, 0, 1)
            item_dict['centroids'] = self.range_normalize(item_dict[
                'centroids'], min_b, max_b, 0, 1)
            if query_points is not None:
                tt = default_timer()
                try:
                    distance, closest = self.compute_distances(mesh,
                        query_points, are_watertight)
                except:
                    deleted_meshes.append(mesh_ind[i])
                    print(
                        f'{i}-th mesh is empty and will not be added to the dataset'
                        )
                    continue
                self.time_to_distance += default_timer() - tt
                item_dict['distance'] = np.expand_dims(distance, -1)
                item_dict['closest_points'] = closest
                item_dict['closest_points'] = self.range_normalize(item_dict
                    ['closest_points'], min_b, max_b, 0, 1)
            data.append(item_dict)
        self.time_to_distance /= len(meshes)
        del meshes
        n_train -= len(deleted_meshes)
        print(f'deleted_meshes={deleted_meshes!r}')
        min_dist, max_dist = self.get_bounds_from_data(data[0:n_train],
            'distance')
        min_area, max_area = self.get_bounds_from_data(data[0:n_train],
            'triangle_areas')
        for data_dict in data:
            data_dict['distance'] = self.range_normalize(data_dict[
                'distance'], min_dist, max_dist, 1e-06, 1)
            data_dict['normalized_triangle_areas'] = self.range_normalize(
                data_dict['triangle_areas'], min_area, max_area, 1e-06, 1)
        for data_dict in data:
            for key in data_dict:
                data_dict[key] = paddle.to_tensor(data=data_dict[key]).to(
                    'float32')
        if attributes is not None:
            for j, ind in enumerate(mesh_ind):
                ...
                for attr in attributes:
                    path = str(data_dir / (item_dir_name + ind + '/' + attr +
                        '.npy'))
                    data[j][attr] = paddle.to_tensor(data=np.load(path))
                    if isinstance(data[j][attr], paddle.Tensor):
                        data[j][attr] = data[j][attr].to('float32')
            normalizer_keys = []
            for attr in attributes:
                if isinstance(data[0][attr], paddle.Tensor):
                    normalizer_keys.append(attr)
            self.normalizers = UnitGaussianNormalizer.from_dataset(data,
                dim=[1], keys=normalizer_keys)
            for attr in normalizer_keys:
                for j in range(len(data)):
                    data_elem = data[j][attr]
                    if tuple(data_elem.shape)[0] != 1:
                        data_elem = data_elem.unsqueeze(axis=0)
        ...
        query_points = paddle.to_tensor(data=query_points).to('float32')
