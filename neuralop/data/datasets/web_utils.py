...
def download_from_url(url: str, root: Union[str, Path], filename: Optional[
    Union[str, Path]]=None, md5: Optional[str]=None, size: Optional[int]=
    None, chunk_size: Optional[int]=256 * 64, extract_tars: bool=True) ->None:
...
def download_from_zenodo_record(record_id: str, root: Union[str, Path],
    ...
    for file_record in response_json['files']:
        ...
        if fname in files_to_download:
            download_from_url(url=file_record['links']['self'], md5=
                file_record['checksum'][4:], size=file_record['size'], root
                =root, filename=fname, extract_tars=True)
