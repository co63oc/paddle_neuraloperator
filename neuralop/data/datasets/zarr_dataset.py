...
class ZarrDataset(paddle.io.Dataset):
    """PDE h5py dataset"""

    def __init__(self, filename, resolution=128, transform_x=None,
        transform_y=None, n_samples=None):
        resolution_to_step = {(128): 8, (256): 4, (512): 2, (1024): 1}
        try:
            subsample_step = resolution_to_step[resolution]
        except KeyError:
            raise ValueError(
                f'Got resolution={resolution}, expected one of {resolution_to_step.keys()}'
                )
        ...
        if n_samples is not None:
            ...
            self.n_samples = tuple(data.shape)[0]
            del data

    def attrs(self, array_name, name):
        data = zarr.open(self.filename, mode='r', synchronizer=zarr.
            ThreadSynchronizer())
    ...
    def data(self):
        if self._data is None:
            self._data = zarr.open(self.filename, mode='r', synchronizer=
    ...
    def __getitem__(self, idx):
        ...
        y = paddle.to_tensor(data=y, dtype='float32').unsqueeze(axis=0)
        if self.transform_x:
    ...
    def __getitems__(self, idx):
        if paddle.is_tensor(x=idx):
            idx = idx.tolist()
        x = paddle.to_tensor(data=[self.data['x'][i, ::self.subsample_step,
            ::self.subsample_step] for i in idx], dtype='float32')
        y = paddle.to_tensor(data=[self.data['y'][i, ::self.subsample_step,
            ::self.subsample_step] for i in idx], dtype='float32')
        if self.transform_x:
            x = self.transform_x(x)
        if self.transform_y:
