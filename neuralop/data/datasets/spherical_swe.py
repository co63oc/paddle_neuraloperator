...
def load_spherical_swe(n_train, n_tests, batch_size, test_batch_sizes,
    train_resolution=(256, 512), test_resolutions=[(256, 512)], device=
    paddle.CPUPlace()):
    ...
    train_dataset = SphericalSWEDataset(dims=train_resolution, num_examples
    ...
    test_loaders = dict()
    for res, n_test, test_batch_size in zip(test_resolutions, n_tests,
        test_batch_sizes):
        print(
            f'Loading test dataloader at resolution {res} with {n_test} samples and batch-size={test_batch_size}'
            )
        test_dataset = SphericalSWEDataset(dims=res, num_examples=n_test,
            device=device)
        test_loader = paddle.io.DataLoader(dataset=test_dataset, batch_size
            =test_batch_size, shuffle=True, num_workers=0)
...
class SphericalSWEDataset(paddle.io.Dataset):
    ...
    def __init__(self, dt=3600, dims=(256, 512), initial_condition='random',
        ...
        self.solver = ShallowWaterSolver(self.nlat, self.nlon, dt_solver,
            lmax=lmax, mmax=mmax, grid='equiangular').to(self.device).float()
