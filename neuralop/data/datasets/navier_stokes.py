...
class NavierStokesDataset(PTDataset):
    ...
    def __init__(self, root_dir: Union[Path, str], n_train: int, n_tests:
        ...
        if download:
            ...
            for res in resolutions:
                if (f'nsforcing_train_{res}.pt' not in
                    already_downloaded_files or f'nsforcing_test_{res}.pt'
                     not in already_downloaded_files):
        ...
        super().__init__(root_dir=root_dir, n_train=n_train, n_tests=
            n_tests, dataset_name='nsforcing', batch_size=batch_size,
...
def load_navier_stokes_pt(n_train, n_tests, batch_size, test_batch_sizes,
    data_root=example_data_root, train_resolution=128, test_resolutions=[
    128], encode_input=False, encode_output=True, encoding='channel-wise',
    channel_dim=1, subsampling_rate=None):
    dataset = NavierStokesDataset(root_dir=data_root, n_train=n_train,
        n_tests=n_tests, batch_size=batch_size, test_batch_sizes=
        test_batch_sizes, train_resolution=train_resolution,
        test_resolutions=test_resolutions, encode_input=encode_input,
        encode_output=encode_output, encoding=encoding, channel_dim=
        channel_dim, subsampling_rate=subsampling_rate)
