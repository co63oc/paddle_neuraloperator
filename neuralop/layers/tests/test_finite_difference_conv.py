...
def test_convergence_FiniteDifferenceConvolution_subtract_middle(resolution):
    ...
    channels = [paddle.sum(x=coeff[i] * paddle.square(x=grid), axis=1) for i in range(num_channels)]
    ...
    for k in range(num_channels):
        ...
        for i in range(kernel_size):
            for j in range(kernel_size):
                direction_k += weight[k, i, j] * paddle.to_tensor(
                    data=[[[[i - kernel_size // 2, j - kernel_size // 2]]]]
                ).to(device)
        direction_k = direction_k.moveaxis(source=-1, destination=1).tile(
            repeat_times=[1, 1, resolution, resolution]
        )
        theoretical_value += 2 * coeff[k] * paddle.sum(x=grid * direction_k, axis=1)
    error = (
        1
        / (resolution - 2)
        * paddle.linalg.norm(
            x=diff_block_output.squeeze()[1:-1, 1:-1] - theoretical_value.squeeze()[1:-1, 1:-1]
        ).item()
    )
