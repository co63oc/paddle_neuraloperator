...
def test_native_segcsr_shapes(batch_size):
    ...
    while sum(nbrhd_sizes) < n_pts:
        nbrhd_sizes.append(paddle.randint(low=0, high=max_nbrhd_size + 1, shape=(1,)))
        max_nbrhd_size = min(max_nbrhd_size, n_pts - sum(nbrhd_sizes))
    indptr = paddle.cumsum(x=paddle.to_tensor(data=nbrhd_sizes, dtype="int64"), axis=0)
    if batch_size > 1:
    ...
    if batch_size == 1:
        ...
        assert tuple(out.shape) == (batch_size, tuple(indptr.shape)[1] - 1, n_channels)
...
def test_native_segcsr_reductions():
    ...
    diff = out_sum - (
        paddle.to_tensor(data=[[3, 5, 2]]).T.astype(paddle.float32) * paddle.ones(shape=[3, 3])
    )
    if not diff.nonzero().is_empty():
