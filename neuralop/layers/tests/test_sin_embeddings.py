...
def test_NeRFEmbedding():
    ...
    unbatched_inputs = paddle.arange(end=in_channels).astype(paddle.float32) * paddle.to_tensor(
        data=[[1.0], [0.5]]
    )
    embeds = nerf_embed(unbatched_inputs.to(device))
    true_outputs = paddle.zeros(shape=[n_in, in_channels * num_freqs * 2]).to(device)
    ...
    batched_inputs = paddle.stack(
        x=[
            paddle.arange(end=in_channels).astype(paddle.float32)
            * paddle.to_tensor(data=[[1.0], [0.5]])
        ]
        * batch_size
    )
    embeds = nerf_embed(batched_inputs.to(device))
    true_outputs = paddle.zeros(shape=[batch_size, n_in, in_channels * num_freqs * 2]).to(device)
    # True values are (sin(2^0 * pi * p), cos(2^0 * pi * p), ... cos(2^(L-1) * pi * p))
    for channel in range(in_channels):
        for wavenumber in range(num_freqs):
            for i in range(2):
