...
def test_FNOBlock_complex_data(n_dim):
    ...
    mlp_expansion = 0.5
    mlp_skip = 'linear'
    block = FNOBlocks(3, 4, modes[:n_dim], n_layers=1, use_mlp=True,
        mlp_dropout=mlp_dropout, mlp_expansion=mlp_expansion, mlp_skip=
        mlp_skip, complex_data=True)
    x = paddle.randn(shape=[2, 3, *size[:n_dim]], dtype='complex64')
    res = block(x)
    assert list(tuple(res.shape)[2:]) == size[:n_dim]
