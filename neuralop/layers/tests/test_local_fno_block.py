...
def test_LocalFNOBlock_output_scaling_factor():
    ...
    mlp_skip = 'linear'
    for dim in [1, 2, 3]:
        block = LocalFNOBlocks(3, 4, max_n_modes[:dim], max_n_modes=
            max_n_modes[:dim], n_layers=1, diff_layers=[True])
        ...
        assert block.convs.n_modes[:-1] == max_n_modes[:dim - 1]
        assert block.convs.n_modes[-1] == max_n_modes[dim - 1] // 2 + 1
        block = LocalFNOBlocks(3, 4, n_modes[:dim], n_layers=1, diff_layers
            =[True], output_scaling_factor=0.5, use_mlp=True, mlp_dropout=
        ...
        block = LocalFNOBlocks(3, 4, n_modes[:dim], n_layers=1, diff_layers
            =[True], output_scaling_factor=2, use_mlp=True, mlp_dropout=
            mlp_dropout, mlp_expansion=mlp_expansion, mlp_skip=mlp_skip)
        x = paddle.randn(shape=[2, 3, *size[:dim]])
        res = block(x)
...
def test_LocalFNOBlock_norm(norm):
    ...
    dim = 2
    ada_in_features = 4
    block = LocalFNOBlocks(3, 4, modes[:dim], n_layers=1, diff_layers=[True
        ], use_mlp=True, norm=norm, ada_in_features=ada_in_features,
