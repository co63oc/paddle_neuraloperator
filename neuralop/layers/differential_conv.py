...
class FiniteDifferenceConvolution(paddle.nn.Layer):
    ...
    def __init__(
        ...
        kernel_size=3,
        groups=1,
        # padding="periodic",   # paddle do not support default "periodic", so change to "zeros"
        padding="zeros",
        implementation="subtract_middle",
    ):
        super().__init__()
        conv_module = getattr(paddle.nn, f"Conv{num_dim}D")
        ...
        if implementation == "subtract_all":
            self.conv_kernel = paddle.base.framework.EagerParamBase.from_tensor(
                tensor=paddle.randn(
                    shape=[
                        out_channels,
                        in_channels // groups,
    ...
    def _forward_subtract_middle(self, x, grid_width):
        ...
        conv_sum = paddle.sum(
            x=self.conv.weight,
            axis=tuple([i for i in range(2, 2 + self.num_dim)]),
            keepdim=True,
        )
    ...
    def _forward_subtract_all(self, x, grid_width):
        x_pad = paddle.nn.functional.pad(
            x=x,
            pad=(self.pad_size, self.pad_size, self.pad_size, self.pad_size),
            mode=self.padding_mode,
            pad_from_left_axis=False,
        )
        conv_mean = paddle.mean(
            x=self.conv_kernel,
            axis=tuple([i for i in range(2, 2 + self.num_dim)]),
            keepdim=True,
        )
        conv_mean = conv_mean.tile(
            repeat_times=[(1) for _ in range(len(tuple(conv_mean.shape)) - self.num_dim)]
            + [self.kernel_size for _ in range(self.num_dim)]
        )
        conv_x = self.F_conv_module(
            x=x_pad,
            weight=self.conv_kernel - conv_mean,
            padding="valid",
            groups=self.groups,
        )
