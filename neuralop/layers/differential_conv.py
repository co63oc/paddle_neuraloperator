...
class FiniteDifferenceConvolution(paddle.nn.Layer):
    ...
    def __init__(self, in_channels, out_channels, num_dim, kernel_size=3,
        groups=1, padding='periodic', implementation='subtract_middle'):
        super().__init__()
        ...
        if padding == 'periodic':
            ...
            raise NotImplementedError(
                'Desired padding mode is not currently supported')
        self.pad_size = kernel_size // 2
        self.implementation = implementation
        if implementation == 'subtract_all':
            self.conv_kernel = (paddle.base.framework.EagerParamBase.
                from_tensor(tensor=paddle.randn(shape=[out_channels, 
            ...
            self.conv = conv_module(in_channels, out_channels, kernel_size=
                kernel_size, padding='same', padding_mode=self.padding_mode,
                bias=False, groups=groups)
            self.weight = self.conv.weight
        else:
            raise NotImplementedError(
                'Desired implementation is not currently supported')

    def _forward_subtract_middle(self, x, grid_width):
        conv = self.conv(x)
        conv_sum = paddle.sum(x=self.conv.weight, axis=tuple([i for i in
            range(2, 2 + self.num_dim)]), keepdim=True)
    ...
    def _forward_subtract_all(self, x, grid_width):
        x_pad = paddle.nn.functional.pad(x=x, pad=(self.pad_size, self.
            pad_size, self.pad_size, self.pad_size), mode=self.padding_mode,
            pad_from_left_axis=False)
        conv_mean = paddle.mean(x=self.conv_kernel, axis=tuple([i for i in
            range(2, 2 + self.num_dim)]), keepdim=True)
        conv_mean = conv_mean.tile(repeat_times=[(1) for _ in range(len(
            tuple(conv_mean.shape)) - self.num_dim)] + [self.kernel_size for
            _ in range(self.num_dim)])
        conv_x = self.F_conv_module(input=x_pad, weight=self.conv_kernel -
            conv_mean, padding='valid', groups=self.groups)
