...
class AttentionKernelIntegral(paddle.nn.Layer):
    ...
    The kernel integral transform will be numerically computed as:
        \\int_{Omega} k(x, y) * f(y) dy ppox \\sum_{j=1}^M * k(x, y_j) * f(y_j) * w(y_j)
    ...
    def __init__(self, in_channels, out_channels, n_heads, head_n_channels,
        project_query=True):
        super().__init__()
        ...
        if project_query:
            self.to_q = paddle.nn.Linear(in_features=in_channels,
                out_features=head_n_channels * n_heads, bias_attr=False)
        else:
            self.to_q = paddle.nn.Identity()
        self.to_k = paddle.nn.Linear(in_features=in_channels, out_features=
            head_n_channels * n_heads, bias_attr=False)
        self.k_norm = paddle.nn.InstanceNorm1D(num_features=head_n_channels,
            weight_attr=False, bias_attr=False)
        self.v_norm = paddle.nn.InstanceNorm1D(num_features=head_n_channels,
            weight_attr=False, bias_attr=False)
        self.to_v = paddle.nn.Linear(in_features=in_channels, out_features=
            head_n_channels * n_heads, bias_attr=False)
        self.to_out = paddle.nn.Linear(in_features=head_n_channels *
            n_heads, out_features=out_channels
            ) if head_n_channels * n_heads != out_channels else paddle.nn.Identity(
            )
        ...
        for param in weight.parameters():
            if param.ndim > 1:
                for h in range(self.n_heads):
                    init_fn(param[h * self.head_n_channels:(h + 1) * self.
                        head_n_channels, :], gain=self.init_gain)
                    if self.head_n_channels == self.in_channels:
                        diagonal_bias = self.diagonal_weight * paddle.diag(x
                            =paddle.ones(shape=param.shape[-1], dtype=
                            'float32'))
                        param.data[h * self.head_n_channels:(h + 1) * self.
                            head_n_channels, :] += diagonal_bias
    ...
    def forward(self, u_src, pos_src, positional_embedding_module=None,
        u_qry=None, pos_qry=None, weights=None, associative=True,
        return_kernel=False):
        """
        ...
        if return_kernel and associative:
            raise ValueError(
                'Cannot get kernel matrix when associative is set to True')
        batch_size, num_grid_points = tuple(u_src.shape)[:2]
        pos_dim = tuple(pos_src.shape)[-1]
        q = self.to_q(u_qry)
        k = self.to_k(u_src)
        v = self.to_v(u_src)
        ...
        if positional_embedding_module is not None:
            if pos_dim == 2:
                ...
                k_freqs_1 = k_freqs_1.unsqueeze(axis=1).tile(repeat_times=[
                    1, self.n_heads, 1, 1])
                k_freqs_2 = k_freqs_2.unsqueeze(axis=1).tile(repeat_times=[
                    1, self.n_heads, 1, 1])
                if pos_qry is None:
                    q_freqs_1 = k_freqs_1
                    q_freqs_2 = k_freqs_2
                else:
                    q_freqs_1 = positional_embedding_module.forward(pos_qry
                        [..., 0])
                    q_freqs_2 = positional_embedding_module.forward(pos_qry
                        [..., 1])
                    q_freqs_1 = q_freqs_1.unsqueeze(axis=1).tile(repeat_times
                        =[1, self.n_heads, 1, 1])
                    q_freqs_2 = q_freqs_2.unsqueeze(axis=1).tile(repeat_times
                        =[1, self.n_heads, 1, 1])
                q = positional_embedding_module.apply_2d_rotary_pos_emb(q,
                    q_freqs_1, q_freqs_2)
                k = positional_embedding_module.apply_2d_rotary_pos_emb(k,
                    k_freqs_1, k_freqs_2)
            elif pos_dim == 1:
                k_freqs = positional_embedding_module.forward(pos_src[..., 0])
                k_freqs = k_freqs.unsqueeze(axis=1).tile(repeat_times=[
                    batch_size, self.n_heads, 1, 1])
                if pos_qry is None:
                    q_freqs = k_freqs
                else:
                    q_freqs = positional_embedding_module.forward(pos_qry[
                        ..., 0])
                    q_freqs = q_freqs.unsqueeze(axis=1).tile(repeat_times=[
                        batch_size, self.n_heads, 1, 1])
                q = positional_embedding_module.apply_1d_rotary_pos_emb(q,
                    q_freqs)
                k = positional_embedding_module.apply_1d_rotary_pos_emb(k,
                    k_freqs)
            else:
                raise ValueError(
        ...
        if associative:
            dots = paddle.matmul(x=k.transpose(perm=paddle_aux.
                transpose_aux_func(k.ndim, -1, -2)), y=v)
            u = paddle.matmul(x=q, y=dots) * weights
        else:
            kxy = paddle.matmul(x=q, y=k.transpose(perm=paddle_aux.
                transpose_aux_func(k.ndim, -1, -2)))
            u = paddle.matmul(x=kxy, y=v) * weights
        u = u.transpose(perm=[0, 2, 1, 3]).contiguous().view(batch_size,
            num_grid_points, self.n_heads * self.head_n_channels)
        u = self.to_out(u)
        if return_kernel:
            return u, kxy
        return u
