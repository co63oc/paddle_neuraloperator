...
class IntegralTransform(paddle.nn.Layer):
    ...
    A(x) : A subset of all points y (depending on        each x) over which to integrate
    ...
    Parameters
    ----------
    channel_mlp : paddle.nn.Layer, default None
    ...
    channel_mlp_non_linearity : callable, default paddle.nn.functional.gelu
    ...
    use_paddle_scatter : bool, default 'True'
        Whether to use paddle_scatter's implementation of
        segment_csr or our native Paddle version. paddle_scatter
        should be installed by default, but there are known versioning
        issues on some linux builds of CPU-only Paddle. Try setting
        to False if you experience an error from paddle_scatter.
    """
    ...
    def forward(self, y, neighbors, x=None, f_y=None, weights=None):
        ...
        x : paddle.Tensor of shape [m, d2], default None
            ...
            x = y.
        f_y : paddle.Tensor of shape [batch, n, d3] or [n, d3], default None
        ...
        Output
        ----------
        out_features : paddle.Tensor of shape [batch, m, d4] or [m, d4]
        ...
        if f_y is not None and (
            self.transform_type == "nonlinear_kernelonly" or self.transform_type == "nonlinear"
        ):
            if batched:
                # repeat agg features for every example in the batch
                agg_features = agg_features.tile(
                    repeat_times=[batch_size] + [1] * agg_features.ndim
                )
        ...
        if weights is not None:
            ...
            if batched:
                nbr_weights = nbr_weights.tile(repeat_times=[batch_size] + [1] * nbr_weights.ndim)
        ...
        if batched:
            splits = splits.tile(repeat_times=[batch_size] + [1] * splits.ndim)
        out_features = segment_csr(
            rep_features, splits, reduce=reduction, use_scatter=self.use_paddle_scatter
        )
        return out_features
