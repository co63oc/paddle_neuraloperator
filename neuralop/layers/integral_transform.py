...
class IntegralTransform(paddle.nn.Layer):
    ...
    def __init__(self, channel_mlp=None, channel_mlp_layers=None,
        channel_mlp_non_linearity=paddle.nn.functional.gelu, transform_type
        ='linear', use_torch_scatter=True):
        super().__init__()
        ...
        if channel_mlp is None:
            self.channel_mlp = LinearChannelMLP(layers=channel_mlp_layers,
    ...
    def forward(self, y, neighbors, x=None, f_y=None, weights=None):
        ...
        if f_y is not None and (self.transform_type ==
            ...
            if batched:
                agg_features = agg_features.tile(repeat_times=[batch_size] +
                    [1] * agg_features.ndim)
        ...
        if weights is not None:
            ...
            if batched:
                nbr_weights = nbr_weights.tile(repeat_times=[batch_size] + 
                    [1] * nbr_weights.ndim)
