...
class SpectralConv(BaseSpectralConv):
    ...
    def __init__(self, in_channels, out_channels, n_modes, max_n_modes=None,
        bias=True, n_layers=1, separable=False, output_scaling_factor:
        Optional[Union[Number, List[Number]]]=None, fno_block_precision=
        'full', rank=0.5, factorization=None, implementation=
        'reconstructed', fixed_rank_modes=False, joint_factorization=False,
        decomposition_kwargs: Optional[dict]=None, complex_data: bool=False,
        init_std='auto', fft_norm='forward', device=None, dtype=None):
        ...
        tensor_kwargs = (decomposition_kwargs if decomposition_kwargs is not
            None else {})
        if factorization is None:
            self.weight = paddle.nn.LayerList(sublayers=[paddle.to_tensor(
                data=weight_shape, dtype='complex64') for _ in range(n_layers)]
                )
            ...
            self.weight = tltorch.factorized_tensors.core.FactorizedTensor.new(
                (n_layers, *weight_shape), rank=self.rank, factorization=
                factorization, fixed_rank_modes=fixed_rank_modes, dtype=
                'complex64', **tensor_kwargs)
            ...
            self.weight = paddle.nn.LayerList(sublayers=[tltorch.
                ...
                fixed_rank_modes=fixed_rank_modes, **tensor_kwargs, dtype=
                'complex64') for _ in range(n_layers)])
            for w in self.weight:
                w.normal_(mean=0, std=init_std)
    ...
    def forward(self, x: paddle.Tensor, indices=0, output_shape: Optional[
        ...
        if self.complex_data:
            x = paddle.fft.fftn(x=x, norm=self.fft_norm, axes=fft_dims)
        else:
...
>>>>>>            out_dtype = torch.chalf
        ...
        out_fft = paddle.zeros(shape=[batchsize, self.out_channels, *
            fft_size], dtype=out_dtype)
        starts = [(max_modes - min(size, n_mode)) for size, n_mode,
            max_modes in zip(fft_size, self.n_modes, self.max_n_modes)]
        if self.separable:
            slices_w = [slice(None)]
        else:
            slices_w = [slice(None), slice(None)]
        if self.complex_data:
            slices_w += [(slice(start // 2, -start // 2) if start else
                slice(start, None)) for start in starts]
        else:
            slices_w += [(slice(start // 2, -start // 2) if start else
                slice(start, None)) for start in starts[:-1]]
            slices_w += [slice(None, -starts[-1]) if starts[-1] else slice(
                None)]
        ...
        if self.separable:
            ...
            weight_start_idx = 2
        starts = [(size - min(size, n_mode)) for size, n_mode in zip(list(
            tuple(x.shape)[2:]), list(tuple(weight.shape)[weight_start_idx:]))]
        slices_x = [slice(None), slice(None)]
        if self.complex_data:
            slices_x += [(slice(start // 2, -start // 2) if start else
                slice(start, None)) for start in starts]
        else:
            slices_x += [(slice(start // 2, -start // 2) if start else
                slice(start, None)) for start in starts[:-1]]
            slices_x += [slice(None, -starts[-1]) if starts[-1] else slice(
                None)]
        out_fft[slices_x] = self._contract(x[slices_x], weight, separable=
            self.separable)
        if self.output_scaling_factor is not None and output_shape is None:
        ...
        if self.complex_data:
            x = paddle.fft.ifftn(x=out_fft, s=mode_sizes, axes=fft_dims,
                norm=self.fft_norm)
        else:
            x = paddle.fft.irfftn(x=out_fft, s=mode_sizes, axes=fft_dims,
                norm=self.fft_norm)
...
class SpectralConv1d(SpectralConv):
    ...
    def forward(self, x, indices=0):
        ...
        out_fft = paddle.zeros(shape=[batchsize, self.out_channels, width //
            2 + 1], dtype='complex64')
        slices = slice(None), slice(None), slice(None, self.n_modes[0])
        out_fft[slices] = self._contract(x[slices], self._get_weight(
            indices)[slices], separable=self.separable)
...
class SpectralConv2d(SpectralConv):
    ...
    def forward(self, x, indices=0):
        ...
        x = paddle.fft.rfft2(x=x.astype(dtype='float32'), norm=self.
            fft_norm, axes=(-2, -1))
        out_fft = paddle.zeros(shape=[batchsize, self.out_channels, height,
            width // 2 + 1], dtype=x.dtype)
        slices0 = slice(None), slice(None), slice(self.n_modes[0] // 2), slice(
            self.n_modes[1])
        slices1 = slice(None), slice(None), slice(-self.n_modes[0] // 2, None
            ), slice(self.n_modes[1])
        print(
            f'2D: x[slices0].shape={tuple(x[slices0].shape)!r}, self._get_weight(indices)[slices0].shape={tuple(self._get_weight(indices)[slices0].shape)!r}, self._get_weight(indices).shape={tuple(self._get_weight(indices).shape)!r}'
            )
        """Upper block (truncate high frequencies)."""
        out_fft[slices0] = self._contract(x[slices0], self._get_weight(
            indices)[slices1], separable=self.separable)
        """Lower block"""
        out_fft[slices1] = self._contract(x[slices1], self._get_weight(
            indices)[slices0], separable=self.separable)
        if self.output_scaling_factor is not None:
...
class SpectralConv3d(SpectralConv):
    ...
    def forward(self, x, indices=0):
        ...
        slices0 = slice(None), slice(None), slice(self.n_modes[0] // 2), slice(
            self.n_modes[1] // 2), slice(self.n_modes[2])
        slices1 = slice(None), slice(None), slice(self.n_modes[0] // 2), slice(
            -self.n_modes[1] // 2, None), slice(self.n_modes[2])
        slices2 = slice(None), slice(None), slice(-self.n_modes[0] // 2, None
            ), slice(self.n_modes[1] // 2), slice(self.n_modes[2])
        slices3 = slice(None), slice(None), slice(-self.n_modes[0] // 2, None
            ), slice(-self.n_modes[1] // 2, None), slice(self.n_modes[2])
        """Upper block -- truncate high frequencies."""
        out_fft[slices0] = self._contract(x[slices0], self._get_weight(
            indices)[slices3], separable=self.separable)
        """Low-pass filter for indices 2 & 4, and high-pass filter for index 3."""
        out_fft[slices1] = self._contract(x[slices1], self._get_weight(
            indices)[slices2], separable=self.separable)
        """Low-pass filter for indices 3 & 4, and high-pass filter for index 2."""
        out_fft[slices2] = self._contract(x[slices2], self._get_weight(
            indices)[slices1], separable=self.separable)
        """Lower block -- low-cut filter in indices 2 & 3
        and high-cut filter in index 4."""
        out_fft[slices3] = self._contract(x[slices3], self._get_weight(
            indices)[slices0], separable=self.separable)
        if self.output_scaling_factor is not None:
            width = round(width * self.output_scaling_factor[0])
            height = round(height * self.output_scaling_factor[1])
            depth = round(depth * self.output_scaling_factor[2])
        x = paddle.fft.irfftn(x=out_fft, s=(height, width, depth), axes=[-3,
            -2, -1], norm=self.fft_norm)
        if self.bias is not None:
            x = x + self.bias[indices, ...]
        return x
