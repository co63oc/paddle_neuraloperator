...
class SpectralConv(BaseSpectralConv):
    ...
    def __init__(
        ...
        if factorization is None:
            self.weight = paddle.nn.LayerList(
                sublayers=[
                    paddle.to_tensor(data=weight_shape, dtype="complex64") for _ in range(n_layers)
                ]
            )
    ...
    def forward(self, x: paddle.Tensor, indices=0, output_shape: Optional[Tuple[int]] = None):
        ...
        if self.fno_block_precision in ["half", "mixed"]:
            # out_dtype = paddle.chalf
            raise NotImplementedError("complex32 not support")
        else:
            out_dtype = "complex64"
        out_fft = paddle.zeros(shape=[batchsize, self.out_channels, *fft_size], dtype=out_dtype)
        ...
        weight = self._get_weight(indices)[tuple(slices_w)]
        ...
        starts = [
            ...
            for size, n_mode in zip(
                list(tuple(x.shape)[2:]), list(tuple(weight.shape)[weight_start_idx:])
            )
        ]
        ...
        out_fft[tuple(slices_x)] = self._contract(
            x[tuple(slices_x)], weight, separable=self.separable
        )
        if self.output_scaling_factor is not None and output_shape is None:
            mode_sizes = tuple(
        ...
        if self.complex_data:
            x = paddle.fft.ifftn(x=out_fft, s=mode_sizes, axes=fft_dims, norm=self.fft_norm)
        else:
...
class SpectralConv2d(SpectralConv):
    ...
    def forward(self, x, indices=0):
        ...
        x = paddle.fft.rfft2(x=x.astype(dtype="float32"), norm=self.fft_norm, axes=(-2, -1))
...
class SpectralConv3d(SpectralConv):
    ...
    def forward(self, x, indices=0):
        ...
        x = paddle.fft.irfftn(
            x=out_fft, s=(height, width, depth), axes=[-3, -2, -1], norm=self.fft_norm
        )
        if self.bias is not None:
            x = x + self.bias[indices, ...]
        return x
