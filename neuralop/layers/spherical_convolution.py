...
class SHT(paddle.nn.Layer):
    ...
    def sht(self, x, s=None, norm="ortho", grid="equiangular"):
        *_, height, width = tuple(x.shape)  # height = latitude, width = longitude
        if s is None:
            if grid == "equiangular":
    ...
    def isht(self, x, s=None, norm="ortho", grid="equiangular"):
        *_, modes_height, modes_width = tuple(x.shape)  # height = latitude, width = longitude
        if s is None:
            if grid == "equiangular":
                width = modes_width * 2
            else:
