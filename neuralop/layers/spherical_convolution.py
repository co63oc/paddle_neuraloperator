...
def _contract_dense_separable(x, weight, separable=True, dhconv=False):
    ...
    if dhconv:
        return x * weight.unsqueeze(axis=-1)
    return x * weight
...
def _contract_cp(x, cp_weight, separable=False, dhconv=True):
    ...
    if dhconv:
        factor_syms += [(xs + rank_sym) for xs in x_syms[2:-1]]
    else:
        factor_syms += [(xs + rank_sym) for xs in x_syms[2:]]
    eq = x_syms + ',' + rank_sym + ',' + ','.join(factor_syms
        ) + '->' + ''.join(out_syms)
...
def _contract_tucker(x, tucker_weight, separable=False, dhconv=False):
    ...
    if separable:
        ...
        factor_syms += [(xs + rs) for xs, rs in zip(x_syms[2:], core_syms[2:])]
    eq = x_syms + ',' + core_syms + ',' + ','.join(factor_syms
        ) + '->' + ''.join(out_syms)
...
class SHT(paddle.nn.Layer):
    """A wrapper for the Spherical Harmonics transform 

    Allows to call it with an interface similar to that of FFT
    """

    def __init__(self, dtype='float32', device=None):
        super().__init__()
        self.device = device
        self.dtype = dtype
        self._SHT_cache = paddle.nn.LayerDict()
        self._iSHT_cache = paddle.nn.LayerDict()

    def sht(self, x, s=None, norm='ortho', grid='equiangular'):
        *_, height, width = tuple(x.shape)
        if s is None:
            if grid == 'equiangular':
        ...
        cache_key = (
            f'{height}_{width}_{modes_height}_{modes_width}_{norm}_{grid}')
        try:
            sht = self._SHT_cache[cache_key]
        except KeyError:
            sht = RealSHT(nlat=height, nlon=width, lmax=modes_height, mmax=
                modes_width, grid=grid, norm=norm).to(device=x.place).to(dtype
                =self.dtype)
            self._SHT_cache[cache_key] = sht
        return sht(x)

    def isht(self, x, s=None, norm='ortho', grid='equiangular'):
        *_, modes_height, modes_width = tuple(x.shape)
        if s is None:
            if grid == 'equiangular':
                width = modes_width * 2
            else:
        ...
        try:
            ...
            isht = InverseRealSHT(nlat=height, nlon=width, lmax=
                modes_height, mmax=modes_width, grid=grid, norm=norm).to(device
                =x.place).to(dtype=self.dtype)
...
class SphericalConv(BaseSpectralConv):
    ...
    def __init__(self, in_channels, out_channels, n_modes, max_n_modes=None,
        ...
        Optional[Union[Number, List[Number]]]=None, rank=0.5, factorization
        ='cp', implementation='reconstructed', fixed_rank_modes=False,
        joint_factorization=False, decomposition_kwargs=dict(), init_std=
        'auto', sht_norm='ortho', sht_grids='equiangular', device=None,
        dtype='float32'):
        ...
        if joint_factorization:
            self.weight = tltorch.factorized_tensors.core.FactorizedTensor.new(
                (self.n_layers, *weight_shape), rank=self.rank,
                factorization=factorization, fixed_rank_modes=
                fixed_rank_modes, **decomposition_kwargs)
            ...
            self.weight = paddle.nn.LayerList(sublayers=[tltorch.
                ...
                fixed_rank_modes=fixed_rank_modes, **decomposition_kwargs) for
                _ in range(n_layers)])
    ...
    def transform(self, x, layer_index=0, output_shape=None):
        *_, in_height, in_width = tuple(x.shape)
        if self.output_scaling_factor is not None and output_shape is None:
            height = round(in_height * self.output_scaling_factor[
                layer_index][0])
        ...
        if (in_height, in_width) == (height, width) and self.sht_grids[
            layer_index] == self.sht_grids[layer_index + 1]:
            return x
        else:
            coefs = self.sht_handle.sht(x, s=self.n_modes, norm=self.
                sht_norm, grid=self.sht_grids[layer_index])
            return self.sht_handle.isht(coefs, s=(height, width), norm=self
                .sht_norm, grid=self.sht_grids[layer_index + 1])
    ...
    def forward(self, x, indices=0, output_shape=None):
        ...
        if self.output_scaling_factor is not None and output_shape is None:
            ...
            height, width = output_shape[0], output_shape[1]
        out_fft = self.sht_handle.sht(x, s=(self.n_modes[0], self.n_modes[1
            ] // 2), norm=self.sht_norm, grid=self.sht_grids[indices])
        out_fft = self._contract(out_fft[:, :, :self.n_modes[0], :self.
            n_modes[1] // 2], self._get_weight(indices)[:, :, :self.n_modes
            [0]], separable=self.separable, dhconv=True)
        x = self.sht_handle.isht(out_fft, s=(height, width), norm=self.
            sht_norm, grid=self.sht_grids[indices + 1])
