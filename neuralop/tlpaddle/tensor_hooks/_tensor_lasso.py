...
class TensorLasso:
    ...
    def __init__(self, penalty=0.01, clamp_weights=True, threshold=1e-06, normalize_loss=True):
...
class CPLasso(TensorLasso, factorization=CPTensor):
    ...
    def __call__(self, module, input, cp_tensor):
        ...
        with paddle.no_grad():
            if self.clamp_weights:
                weights.data = paddle.clip(x=weights.data, min=-1, max=1)
                setattr(module, "lasso_weights", weights)
            if self.threshold:
                weights.data = paddle.nn.functional.thresholded_relu(
                    x=weights.data, threshold=self.threshold, value=0
                )
                setattr(module, "lasso_weights", weights)
        self.n_element += weights.size
        self._loss = self._loss + self.penalty * paddle.linalg.norm(x=weights, p=1)
        return cp_tensor
    ...
    def apply(self, module):
        ...
        lasso_weights = paddle.base.framework.EagerParamBase.from_tensor(
            tensor=paddle.ones([module.rank], **context)
        )
...
class TuckerLasso(TensorLasso, factorization=TuckerTensor):
    ...
    def __call__(self, module, input, tucker_tensor):
        ...
        with paddle.no_grad():
            for i in range(order):
                if self.clamp_weights:
                    lasso_weights[i].data = paddle.clip(x=lasso_weights[i].data, min=-1, max=1)
                if self.threshold:
                    lasso_weights[i] = paddle.nn.functional.thresholded_relu(
                        x=lasso_weights[i], threshold=self.threshold, value=0
                    )
            setattr(module, "lasso_weights", lasso_weights)
        for weight in lasso_weights:
            self.n_element += weight.size
            self._loss = self._loss + paddle.sum(x=paddle.abs(x=weight))
    ...
    def apply(self, module):
        ...
        lasso_weights = ParameterList(
            [
                paddle.base.framework.EagerParamBase.from_tensor(tensor=paddle.ones([r], **context))
                for r in rank
            ]
        )
        setattr(module, "lasso_weights", lasso_weights)
        ...
        lasso_weights = ParameterList(
            [
                paddle.base.framework.EagerParamBase.from_tensor(
                    tensor=paddle.ones(shape=[1, 1, r])
                )
                for r in rank
            ]
        )
        setattr(module, "lasso_weights", lasso_weights)
        handle = module.register_forward_hook(self)  # noqa
        return module
...
def remove_tensor_lasso(factorized_tensor):
    ...
    Parameters
    ----------
    factorized_tensor : tlpaddle.FactorizedTensor
