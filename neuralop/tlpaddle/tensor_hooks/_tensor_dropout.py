    ...
    def apply(cls, module, proba, min_dim=3, min_values=1, drop_test=False):
        ...
        for k, hook in module._forward_hooks.items():
            if isinstance(hook, cls):
                raise RuntimeError("Cannot register two weight_norm hooks on the same parameter")
        dropout = cls(proba, min_dim=min_dim, min_values=min_values, drop_test=drop_test)
        handle = module.register_forward_post_hook(hook=dropout)
        return handle
...
class TuckerDropout(TensorDropout, factorization=TuckerTensor):
    def _apply_tensor_dropout(self, tucker_tensor, training=True):
        ...
        for rank in tucker_rank:
            idx = tl.arange(rank, device=core.place, dtype="int64")
            if rank > self.min_dim:
                idx = idx[
                    paddle.assign(
                        paddle.bernoulli(x=paddle.ones(shape=rank) * (1 - self.proba)),
                        output=paddle.empty(shape=rank, dtype="bool"),
                    )
                ]
                if len(idx) == 0:
                    idx = paddle.randint(low=0, high=rank, shape=(self.min_values,), dtype="int64")
...
class CPDropout(TensorDropout, factorization=CPTensor):
    def _apply_tensor_dropout(self, cp_tensor, training=True):
        if not self.proba or not training and not self.drop_test:
            return cp_tensor
        rank = cp_tensor.rank
        device = cp_tensor.factors[0].place
        if rank > self.min_dim:
            sampled_indices = tl.arange(rank, device=device, dtype="int64")
            ...
            if len(sampled_indices) == 0:
                sampled_indices = paddle.randint(
                    low=0, high=rank, shape=(self.min_values,), dtype="int64"
                )
...
class TTDropout(TensorDropout, factorization=TTTensor):
    def _apply_tensor_dropout(self, tt_tensor, training=True):
        if not self.proba or not training and not self.drop_test:
            return tt_tensor
        device = tt_tensor.factors[0].place
        sampled_indices = []
        for i, rank in enumerate(tt_tensor.rank[1:]):
            if rank > self.min_dim:
                idx = tl.arange(rank, device=device, dtype="int64")
                idx = idx[
                    paddle.assign(
                        paddle.bernoulli(x=paddle.ones(shape=rank) * (1 - self.proba)),
                        output=paddle.empty(shape=rank, dtype="bool"),
                ...
                idx = tl.arange(rank, device=device, dtype="int64").tolist()
            sampled_indices.append(idx)
        sampled_factors = []
        if training:
