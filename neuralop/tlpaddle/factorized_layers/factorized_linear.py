...
class FactorizedLinear(paddle.nn.Layer):
    ...
    device : Paddle device to use, default is None
    dtype : Paddle dtype, default is None
    """
        ...
        if bias:
            if n_layers == 1:
                self.bias = paddle.base.framework.EagerParamBase.from_tensor(
                    tensor=paddle.empty(shape=self.out_features, dtype=dtype)
                )
                self.has_bias = True
            else:
                self.bias = paddle.base.framework.EagerParamBase.from_tensor(
                    tensor=paddle.empty(shape=(n_layers, self.out_features), dtype=dtype)
                )
    ...
    def reset_parameters(self):
        with paddle.no_grad():
            self.weight.normal_(mean=0, std=math.sqrt(5) / math.sqrt(self.in_features))
    ...
    def forward(self, x, indices=0):
        ...
        if self.checkpointing and not x.stop_gradient:
    ...
    def from_linear(
        ...
        """
        out_features, in_features = tuple(linear.weight.shape)
        if auto_tensorize:
    ...
    def from_linear_list(
        ...
        weight_tensor = paddle.stack(x=[layer.weight.data for layer in linear_list])
    ...
    def __repr__(self):
        msg = f"{self.__class__.__name__}(in_features={self.in_features}, out_features={self.out_features}, weight of size ({self.out_features}, {self.in_features}) tensorized to ({self.out_tensorized_features}, {self.in_tensorized_features}),factorization={self.weight._name}, rank={self.rank}, implementation={self.implementation}"
...
class SubFactorizedLinear(paddle.nn.Layer):
