...
def _ensure_array(layers_shape, order, value, one_per_order=True):
    ...
    if isinstance(value, np.ndarray):
        assert tuple(value.shape) == target_shape
        return value
    if isinstance(value, int):
...
class FactorizedConv(paddle.nn.Layer):
    ...
    def __init__(
        ...
        if isinstance(kernel_size, int):
            ...
            if not isinstance(order, int) or order <= 0:
                raise ValueError(
                    f"order should be the (positive integer) order of the convolutionbut got order={order} of type {type(order)}."
                )
            else:
        ...
        if bias:
            self.bias = paddle.base.framework.EagerParamBase.from_tensor(
                tensor=paddle.empty(shape=[*layers_shape, out_channels], dtype=dtype)
            )
        else:
        ...
        if isinstance(factorization, FactorizedTensor):
            ...
            kernel_shape = factorization_shape_to_kernel_shape(
                factorization._name, tuple(factorization.shape)
            )
        else:
    ...
    def forward(self, x, indices=0):
        ...
        if isinstance(self.n_layers, int):
            if not isinstance(indices, int):
                raise ValueError(
                    f"Expected indices to be in int but got indices={indices}, but this conv was created with n_layers={self.n_layers}."
                )
    ...
    def reset_parameters(self, std=0.02):
        ...
        self.weight = self.weight.normal_(mean=0, std=std)
    ...
    def from_conv(
        ...
        **kwargs,
        ...
        out_channels, in_channels, *kernel_size = tuple(conv_layer.weight.shape)
        ...
        if decompose_weights:
            with paddle.no_grad():
                weight_tensor = paddle.stack(
                    x=[kernel_to_tensor(factorization, layer.weight.data) for layer in conv_list]
                )
    ...
    def transduct(
        ...
        self.kernel_shape = (
            self.kernel_shape[: mode + 2] + (kernel_size,) + self.kernel_shape[mode + 2 :]
        )
        if isinstance(self.weight, CPTensor):
            new_factor = paddle.zeros(shape=[kernel_size, self.weight.rank])
    ...
    def extra_repr(self):
        s = f"in_channels={self.in_channels}, out_channels={self.out_channels}, kernel_size={self.kernel_size}, rank={self.rank}, order={self.order}"
        if self.n_layers == 1:
            s += ", "
...
class SubFactorizedConv(paddle.nn.Layer):
    ...
    def __repr__(self):
        ...
        msg += (
            f"in_channels={self.main_conv.in_channels}, out_channels={self.main_conv.out_channels}"
        )
