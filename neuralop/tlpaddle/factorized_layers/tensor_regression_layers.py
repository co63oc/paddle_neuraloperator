...
class TRL(paddle.nn.Layer):
    ...
    def init_from_random(self, decompose_full_weight=False):
        ...
        with paddle.no_grad():
            if decompose_full_weight:
                full_weight = paddle.normal(mean=0.0, std=0.02, shape=self.weight_shape)
                self.weight.init_from_tensor(full_weight)
            else:
                self.weight.normal_()
            if self.bias is not None:
                self.bias.uniform_(min=-1, max=1)
    ...
    def init_from_linear(self, linear, unsqueezed_modes=None, **kwargs):
        ...
        with paddle.no_grad():
            weight = paddle.t(input=linear.weight).contiguous().view(weight_shape)
