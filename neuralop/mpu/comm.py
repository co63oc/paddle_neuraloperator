...
def get_data_parallel_size():
    if not paddle.distributed.is_initialized():
        ...
        return paddle.distributed.get_world_size(group=_DATA_PARALLEL_GROUP)
...
def get_data_parallel_group():
    assert paddle.distributed.is_initialized(), "Error, initialize paddle.distributed first"
    return _DATA_PARALLEL_GROUP


# model parallel
def get_model_parallel_size():
    if not paddle.distributed.is_initialized() or _MODEL_PARALLEL_GROUP is None:
        return 1
    else:
        return paddle.distributed.get_world_size(group=_MODEL_PARALLEL_GROUP)
...
def init(config, verbose=False):
    ...
    assert (
        model_group_size <= world_size and world_size % model_group_size == 0
    ...
    if world_size > 1:
        if model_group_size > 1:
            ...
            with disable_logging():
                _MODEL_PARALLEL_GROUP = paddle.distributed.new_group(ranks=[world_rank])
                _SPATIAL_PARALLEL_GROUP = _MODEL_PARALLEL_GROUP
                _MATMUL_PARALLEL_GROUP = _MODEL_PARALLEL_GROUP
                _DATA_PARALLEL_GROUP = paddle.distributed.new_group(ranks=list(range(world_size)))
    # barrier
