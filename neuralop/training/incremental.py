...
class IncrementalFNOTrainer(Trainer):
    ...
    def __init__(self, model: paddle.nn.Layer, n_epochs: int, wandb_log:
        ...
        False, incremental_grad: bool=False, incremental_loss_gap: bool=
        False, incremental_grad_eps: float=0.001, incremental_buffer: int=5,
        incremental_max_iter: int=1, incremental_grad_max_iter: int=10,
        ...
        super().__init__(model=model, n_epochs=n_epochs, wandb_log=
            wandb_log, device=device, mixed_precision=mixed_precision,
            data_processor=data_processor, eval_interval=eval_interval,
            log_output=log_output, use_distributed=use_distributed, verbose
            =verbose)
        ...
        assert self.incremental, 'Error: IncrementalTrainer expects at least one incremental algorithm to be True.'
    ...
    def train_one_epoch(self, epoch, train_loader, training_loss):
        ...
        train_err, avg_loss, avg_lasso_loss, epoch_train_time = super(
    ...
    def loss_gap(self, loss):
        ...
        if len(self.loss_list) > 1:
            if abs(self.loss_list[-1] - self.loss_list[-2]
                ) <= self.incremental_loss_eps:
    ...
    def grad_explained(self):
        if not hasattr(self, 'accumulated_grad'):
            self.accumulated_grad = paddle.zeros_like(x=self.model.
                fno_blocks.convs.weight[0])
        ...
        if self.grad_iter <= self.incremental_grad_max_iter:
            ...
            for i in range(self.ndim):
                ...
                strength_vector = []
                for mode_index in range(min(tuple(weight.shape)[1],
                    incremental_modes)):
                    strength = paddle.linalg.norm(x=weight[:, mode_index, :
                        ], p='fro')
                    strength_vector.append(strength)
                expained_ratio = compute_explained_variance(
                    incremental_modes - self.incremental_buffer, paddle.
                    to_tensor(data=strength_vector))
