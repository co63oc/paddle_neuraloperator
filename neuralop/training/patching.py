...
class MultigridPatching2D(paddle.nn.Layer):
    def __init__(self, model, levels=0, padding_fraction=0, use_distributed=False, stitching=True):
        ...
        if self.use_distributed and self.stitching:
            for param in model.parameters():
                param.register_hook(hook=lambda grad: grad * float(comm.get_model_parallel_size()))
    ...
    def _make_mg_patches(self, x):
        ...
        for level in range(1, levels + 1):
            ...
            s2_pad = (
                math.ceil((s2_patched + (2**levels - 1) * s2_stride - x_sub.shape[-1]) / 2.0)
                + padding[1]
            )
            s1_pad = (
                math.ceil((s1_patched + (2**levels - 1) * s1_stride - x_sub.shape[-2]) / 2.0)
                + padding[0]
            )
            if s2_pad > x_sub.shape[-1]:
                ...
                x_sub = paddle.nn.functional.pad(
                    x=x_sub,
                    pad=[x_sub.shape[-1], x_sub.shape[-1], 0, 0],
                    mode="circular",
            ...
            if s1_pad > x_sub.shape[-2]:
                ...
                x_sub = paddle.nn.functional.pad(
                    x=x_sub,
                    pad=[0, 0, x_sub.shape[-2], x_sub.shape[-2]],
                    mode="circular",
                    pad_from_left_axis=False,
                )
                x_sub = paddle.nn.functional.pad(
            ...
            x_sub = x_sub.unfold(axis=-1, size=s2_patched + 2 * padding[1], step=s2_stride)
            x_sub = x_sub.unfold(axis=-3, size=s1_patched + 2 * padding[0], step=s1_stride)
...
def make_patches(x, n, p=0):
    ...
    if p[0] > 0 or p[1] > 0:
        if d == 1:
            x = paddle.nn.functional.pad(x=x, pad=p, mode="circular", pad_from_left_axis=False)
        else:
    ...
    for j in range(d):
        ...
        x = x.unfold(axis=-(2 * j + 1), size=patch_size + 2 * p[-(j + 1)], step=patch_size)
