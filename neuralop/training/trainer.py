...
class Trainer:
    ...
    def __init__(self, *, model: paddle.nn.Layer, n_epochs: int, wandb_log:
        bool=False, device: str='cpu', mixed_precision: bool=False,
        data_processor: paddle.nn.Layer=None, eval_interval: int=1,
        log_output: bool=False, use_distributed: bool=False, verbose: bool=
        False):
        """
        Parameters
        ----------
    ...
    def train(self, train_loader, test_loaders, optimizer, scheduler,
        regularizer=None, training_loss=None, eval_losses=None, save_every:
        int=None, save_best: int=None, save_dir: Union[str, Path]='./ckpt',
        ...
        if self.save_best is not None:
            ...
            assert self.save_best in metrics, f'Error: expected a metric of the form <loader_name>_<metric>, got {save_best}'
        ...
        for epoch in range(self.n_epochs):
            train_err, avg_loss, avg_lasso_loss, epoch_train_time = (self.
                train_one_epoch(epoch, train_loader, training_loss))
            epoch_metrics = dict(train_err=train_err, avg_loss=avg_loss,
                avg_lasso_loss=avg_lasso_loss, epoch_train_time=
                epoch_train_time)
            if epoch % self.eval_interval == 0:
                eval_metrics = self.evaluate_all(epoch=epoch, eval_losses=
                    eval_losses, test_loaders=test_loaders)
    ...
    def train_one_epoch(self, epoch, train_loader, training_loss):
        ...
        if self.verbose and epoch % self.eval_interval == 0:
            self.log_training(epoch=epoch, time=epoch_train_time, avg_loss=
                avg_loss, train_err=train_err, avg_lasso_loss=
                avg_lasso_loss, lr=lr)
        return train_err, avg_loss, avg_lasso_loss, epoch_train_time

    def evaluate_all(self, epoch, eval_losses, test_loaders):
        all_metrics = {}
        for loader_name, loader in test_loaders.items():
            loader_metrics = self.evaluate(eval_losses, loader, log_prefix=
                loader_name)
    ...
    def evaluate(self, loss_dict, data_loader, log_prefix='', epoch=None):
        ...
        errors = {f'{log_prefix}_{loss_name}': (0) for loss_name in
            loss_dict.keys()}
        self.n_samples = 0
        with paddle.no_grad():
            for idx, sample in enumerate(data_loader):
                ...
                if idx == len(data_loader) - 1:
                    return_output = True
                eval_step_losses, outs = self.eval_one_batch(sample,
                    loss_dict, return_output=return_output)
    ...
    def train_one_batch(self, idx, sample, training_loss):
        ...
        if self.data_processor is not None:
            ...
            sample = {k: v.to(self.device) for k, v in sample.items() if
    ...
    def eval_one_batch(self, sample: dict, eval_losses: dict, return_output:
        bool=False):
    ...
    def log_training(self, epoch: int, time: float, avg_loss: float,
        train_err: float, avg_lasso_loss: float=None, lr: float=None):
        ...
        """
        if self.wandb_log:
            values_to_log = dict(train_err=train_err, time=time, avg_loss=
                avg_loss, avg_lasso_loss=avg_lasso_loss, lr=lr)
    ...
    def resume_state_from_dir(self, save_dir):
        ...
        self.model = load_training_state(save_dir=save_dir, save_name=
            save_name, model=self.model, optimizer=self.optimizer,
            regularizer=self.regularizer, scheduler=self.scheduler)
    ...
    def checkpoint(self, save_dir):
        ...
        if self.save_best is not None:
            ...
            save_name = 'model'
        save_training_state(save_dir=save_dir, save_name=save_name, model=
            self.model, optimizer=self.optimizer, scheduler=self.scheduler,
