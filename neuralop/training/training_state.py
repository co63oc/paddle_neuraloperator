...
def load_training_state(
    ...
    optimizer: paddle.nn.Layer = None,
    scheduler: paddle.nn.Layer = None,
    regularizer: paddle.nn.Layer = None,
    ...
    model : nn.Layer
        model to save
    optimizer : nn.Layer, optional
        optimizer object to save, by default None
    scheduler : nn.Layer, optional
        scheduler object to save, by default None
    regularizer : nn.Layer, optional
    ...
    map_location : dict, optional
        ...
        dictionary instructs paddle to load a model from a checkpoint on rank `device_from`
    ...
    if optimizer is not None:
        ...
        if optimizer_pth.exists():
            optimizer.set_state_dict(state_dict=paddle.load(path=str(optimizer_pth)))
        else:
            print(
    ...
    if scheduler is not None:
        ...
        if scheduler_pth.exists():
            scheduler.set_state_dict(state_dict=paddle.load(path=str(scheduler_pth)))
        else:
            print(
    ...
    if regularizer is not None:
        ...
        if regularizer_pth.exists():
            regularizer.set_state_dict(state_dict=paddle.load(path=str(regularizer_pth)))
        else:
            print(
