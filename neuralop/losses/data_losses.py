...
class LpLoss(object):
    ...
    def abs(self, x, y, h=None):
        ...
        diff = const * paddle.linalg.norm(
            x=paddle.flatten(x=x, start_axis=-self.d) - paddle.flatten(x=y, start_axis=-self.d),
            p=self.p,
            axis=-1,
            keepdim=False,
        )
    ...
    def rel(self, x, y):
        ...
        y : paddle.Tensor
            targets
        """
        diff = paddle.linalg.norm(
            x=paddle.flatten(x=x, start_axis=-self.d) - paddle.flatten(x=y, start_axis=-self.d),
            p=self.p,
            axis=-1,
            keepdim=False,
        )
        ynorm = paddle.linalg.norm(
            x=paddle.flatten(x=y, start_axis=-self.d), p=self.p, axis=-1, keepdim=False
        )
        diff = diff / ynorm
        if self.reduce_dims is not None:
            diff = self.reduce_all(diff).squeeze()
...
class H1Loss(object):
    ...
    def abs(self, x, y, h=None):
        ...
        const = math.prod(h)
        diff = const * paddle.linalg.norm(x=dict_x[0] - dict_y[0], p=2, axis=-1, keepdim=False) ** 2
        for j in range(1, self.d + 1):
            diff += (
                const
                * paddle.linalg.norm(x=dict_x[j] - dict_y[j], p=2, axis=-1, keepdim=False) ** 2
            )
        diff = diff**0.5
        if self.reduce_dims is not None:
            diff = self.reduce_all(diff).squeeze()
    ...
    def rel(self, x, y, h=None):
        ...
        if h is None:
            ...
            h = [h] * self.d
        dict_x, dict_y = self.compute_terms(x, y, h)
        diff = paddle.linalg.norm(x=dict_x[0] - dict_y[0], p=2, axis=-1, keepdim=False) ** 2
        ynorm = paddle.linalg.norm(x=dict_y[0], p=2, axis=-1, keepdim=False) ** 2
        for j in range(1, self.d + 1):
            diff += paddle.linalg.norm(x=dict_x[j] - dict_y[j], p=2, axis=-1, keepdim=False) ** 2
            ynorm += paddle.linalg.norm(x=dict_y[j], p=2, axis=-1, keepdim=False) ** 2
...
class PointwiseQuantileLoss(object):
    ...
    def __call__(self, y_pred, y, eps=1e-07, **kwargs):
        ...
        ptavg_loss = ptwise_loss_scaled.view(tuple(ptwise_loss_scaled.shape)[0], -1).mean(
            axis=1, keepdim=True
        )
        if self.reduce_dims is not None:
...
class MSELoss(object):
    ...
    def __init__(self, reductions="sum"):
        ...
        allowed_reductions = ["sum", "mean"]
        assert (
            reductions in allowed_reductions
        ), f"error: expected `reductions` to be one of {allowed_reductions}, got {reductions}"
        self.reductions = reductions

    def __call__(self, y_pred: paddle.Tensor, y: paddle.Tensor, dim: List[int] = None, **kwargs):
        """MSE loss call
        ...
        if self.reductions == "sum":
            return paddle.mean(x=(y_pred - y) ** 2, axis=dim).sum()  # sum of MSEs for each element
